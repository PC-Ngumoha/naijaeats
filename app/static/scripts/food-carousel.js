...
carousels.forEach((carousel) => {
  ...
  carousel.addEventListener("click", (e) => {
    ...
    if (target.matches("#modal-btn")) {
      console.log(`btn-${target.getAttribute("data-food-modal")} clicked`);
    } else {
      console.log(`Modal button not getting clicked`);
    }
  });
...
carousels.forEach((carousel) => {
  ...
  setInterval(() => {
    ...
    arrows[1].classList.toggle(
      "disabled",
      carousel.scrollLeft + carousel.clientWidth >= endPoint
    );
  }, 50);
});
